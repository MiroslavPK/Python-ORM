...
def show_all_authors_with_their_books() -> str:
    ...
    for author in authors:
        ...
        authors_with_books.append(
            f"{author.name} has written - {', '.join(titles)}!"
        )
    # for author in Author.objects.select_related().all().order_by('id'):
    #     if not author.books:
    #         continue
    #     print(author.books.values('title'))
    #     authors_with_books.append(
    #         f"{author.name} has written - {', '.join(author.books.values()[0])}!"
    #     )
...
def get_drivers_with_expired_licenses(due_date: date) -> QuerySet[Driver]:
    ...
    drivers = Driver.objects.filter(licenses__issue_date__gt=expiration_date)
    return drivers
