...
def update_to_512_GB_storage() -> None:
    # Laptop.objects.filter(brand=Q('Asus') | Q('Lenovo')).update(storage=512)
    Laptop.objects.filter(brand__in=['Asus','Lenovo']).update(storage=512)


def update_to_16_GB_memory() -> None:
    # Laptop.objects.filter(brand=Q('Apple') | Q('Dell') | Q('Acer')).update(memory=16)
...
def show_hard_dungeons() -> str:
    ...
    return '\n'.join(str(dng) for dng in hard_dungeons)
...
def update_dungeon_rewards() -> None:
    ...
    Dungeon.objects.update(
        reward = Case(
            When(
                Q(boss_health=500) & ~(Q(location__startswith='E') | Q(location__endswith='s')), 
                then=Value('1000 Gold')
            ),
            When(
                Q(location__startswith='E') & ~(Q(location__endswith='s')), 
                then=Value('New dungeon unlocked')),
            When(
                Q(location__endswith='s'), 
                then=Value('Dragonheart Amulet')),
        )
    )
...
def delete_workouts() -> None:
    Workout.objects.exclude(
        workout_type__in=('Strength', 'Calisthenics')).delete()
